...
function printError(errorMsg) {
	var log = '<span style="color:red;">' + errorMsg + '</span><br/>';
	$('#loginError').empty();
...
function function_Authenticate_user() {
	...
	// For Testing
	if (window.location.hostname === 'localhost') eeFolderPath = 'marketplace/global/en_US';
	var uiURL = '/' + eefolderPath + '/CreateSaml2';
	var formData = $('#form-CreateSaml2').serialize();

	// These need to be run in serial to finish the auth.
	var authRequests = [
		{
			name: 'MGMTUI',
			url: $('#mgmt-ui-url').val() + 'CreateSaml2'
		},
		{
			name: 'MGMTREST',
			url: $('#mgmt-rest-url').val() + 'CreateSaml2'
		},
		{
			name: 'UI',
			url: uiURL // Why is this request being done twice?
		},
		{
			name: 'REST',
			url: $('#rest-url').val() + 'CreateSaml2'
		}
	];

	var formRequest = $.post(uiURL, formData).done(function(result) {
		var returnData = result[0];
		var returnMessage = returnData.returnMessage;
		var returnCode = returnData.returnCode;
		if (returnCode === 200) {
			launchAuthRequests(authRequests, finalRedirect);
		} else if (returnCode >= 300 && returnCode < 400) {
			navigate(returnMessage);
			$.unblockUI();
		} else if (returnCode >= 400 && returnCode < 500) {
			printError(returnMessage);
			$.unblockUI();
		}
	});

	bindErrorHandlerToRequest(formRequest);

	// Each request has the same error handler.
	function bindErrorHandlerToRequest(request, flagName) {
		request.fail(function() {
			if (flagName) {
				log(flag + ' = false');
				printError('Failure In creating Tokens');
			}
			$.unblockUI();
		});
	}

	// Work through each request in series.
	function launchAuthRequests(authRequests, finalRedirect) {
		// If we've completed them all, redirect.
		if (authRequests.length === 0){
			return navigate(finalRedirect);
		}

		// Get the request at the head of the list & execute it.
		var req = authRequests.shift();
		var data = {
			ffe_auth_user: $('#ffe_auth_user').val(),
			ffe_auth_passwd: $('#ffe_auth_passwd').val(),
			actionId: 'CreateUserSaml2'
		};
		// Run this request. When it's done, run the next.
		var postRequest = $.post(req.url, data).done(function(){
			log(req.name + 'Flag = true');
			launchAuthRequests(authRequests, finalRedirect);
		});
		// If we error, print an error & unblock the UI.
		bindErrorHandlerToRequest(postRequest);
	}
}
	...
	try {
		...
		if (agentBrokerSAMLToken && agentBrokerSAMLToken != null && agentBrokerSAMLToken != '') {
			...
			$.ajax({
				...
				type: "POST",
				data: {
					"EE_SAML_ATTRIBUTES_STRING": agentBrokerSAMLToken
				},
				success: function(response) {
		...
		.success(function(result) {
			...
			jQuery.post(url, $('#form-DeleteSaml2').serialize(), function(data) {})
				.error(function() {
					...
					printError('Failure In deleting Tokens');
					console.log('MGMTUIflag = ' + successFlag);
					$.unblockUI();
				})
				.success(function() {
